...
def validation(model, dataloader, device):
	...
	with torch.no_grad():
		model.eval()
		for x, soft_y, real_y, bc in tqdm(dataloader, leave=False, ncols=80):
...
def evaluation():
	...
	dataset = LifeSpanDataset(TEST_DIR, df, input_shape=[160, 192, 160])
	dataloader = DataLoader(dataset, batch_size=4)

	model = DataParallel(SFCN()).to(device)
	model.load_state_dict(torch.load(join(OUTPUT_NETWORK, "age_prediction_model.pt")))
	os.makedirs(OUTPUT_EVAL, exist_ok=True)
	preds, gt = validation(model, dataloader, device)
	print(F.l1_loss(preds, gt))
	print(r2_score(gt.cpu().numpy(), preds.cpu().numpy()))
	with open(join(OUTPUT_EVAL, "out.pkl"), "wb") as f:
